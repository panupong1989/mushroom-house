// Validate setpoints — mirror backend/src/services/validateConfig.ts (docs/03-control-logic.md)
// รับ config ที่ merge ค่าเดิม+ค่าใหม่แล้ว; cross-field ข้ามถ้า key ที่เกี่ยวข้องยังไม่มีค่า
...
export function validateSetpoints(config: Record<string, number>): ValidationError[] {
  ...
  for (const [key, value] of Object.entries(config)) {
    if (Number.isNaN(value)) {
      errors.push({ key, message: `${key} ต้องเป็นตัวเลข` });
      continue;
    }
